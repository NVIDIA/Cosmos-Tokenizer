...
Usage:
    python3 -m cosmos_tokenizer.video_cli \
        ...
        --checkpoint_enc ./pretrained_ckpts/CosmosCausalCV_f4x8x8/encoder.jit \
        --checkpoint_dec ./pretrained_ckpts/CosmosCausalCV_f4x8x8/decoder.jit

    Optionally, you can run the model in pure PyTorch mode:
    python3 -m cosmos_tokenizer.video_cli \
        --video_pattern 'path/to/video/samples/*.mp4' \
        --mode=torch \
        --tokenizer_type=CausalCV \
...
def _parse_args() -> tuple[Namespace, dict[str, Any]]:
    ...
    parser.add_argument(
        "--credentials", type=str, default=None, help="The S3 credentials."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
...
args = _parse_args()
if args.mode == "torch" and args.tokenizer_type not in ["CausalCV", "CausalDV"]:
    logging.error(
        "'torch' backend requires the tokenizer_type of 'CausalCV' or 'CausalDV'."
    )
    sys.exit(1)
init_s3_backend(credentials=args.credentials)
...
def _run_eval() -> None:
    ...
    if args.mode == "torch":
        ...
        tokenizer_config.update(
            dict(spatial_compression=args.spatial_compression)
        )
        tokenizer_config.update(
            dict(temporal_compression=args.temporal_compression)
        )
    else:
        tokenizer_config = None

    logging.info("Loading a torch.jit model from PBSS ...")
    ...
    logging.info(
        f"Looking for files matching video_pattern={args.video_pattern} ..."
    )
    ...
    for filepath in filepaths:
        ...
        output_video = autoencoder(
            batch_video, temporal_window=args.temporal_window
        )[0]
        logging.info("Constructing output filepath ...")
        output_filepath = get_output_filepath(
            filepath, output_dir=args.output_dir
        )
