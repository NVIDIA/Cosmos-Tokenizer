...
import jammy.fileio as easy_io
...
def init_s3_backend(backend: str = "s3", credentials: str = None) -> None:
...
def load_model(
    ...
    full_model, ckpts = _load_pytorch_model(
        jit_filepath, tokenizer_config, device
    )
...
def _load_pytorch_model(
    ...
    if jit_filepath.startswith("s3://"):
        ckpts = easy_io.load(jit_filepath)
    else:
...
def load_jit_model(
    ...
    """
    if jit_filepath.startswith("s3://"):
        model = easy_io.load(jit_filepath)
    else:
...
def save_jit_model(
    ...
    Args:
        ...
        jit_filepath: The filepath to the JIT-compiled model (S3 or local).
    """
    if jit_filepath.startswith("s3://"):
        model = easy_io.dump(model, jit_filepath)
    else:
        torch.jit.save(model, jit_filepath)


def get_filepaths(input_pattern) -> list[str]:
    """Returns a list of filepaths from a pattern, with depth=1."""
    if input_pattern.startswith("s3://"):
        filepaths = []
        media_pattern = os.path.dirname(input_pattern)
        suffix = os.path.splitext(input_pattern)
        pattern_depth = len(media_pattern.replace("s3://", "").split("/"))
        for file_path in easy_io.list_dir_or_file(
            media_pattern, list_dir=False, suffix=suffix
        ):
            file_depth = len(file_path.replace("s3://", "").split("/"))
            if file_depth > pattern_depth:
                break
            _path = f"{media_pattern}/{os.path.basename(file_path)}"
            if easy_io.exists(_path):
...
def read_image(filepath: str) -> np.ndarray:
    """Reads an image from a filepath in S3 or local.

    Args:
        filepath: The filepath to the image. (local or S3)

    Returns:
        The image as a numpy array, layout HxWxC, range [0..255], uint8 dtype.
    """
    if filepath.startswith("s3://"):
        return np.array(easy_io.load(filepath))
    else:
...
def read_video(filepath: str) -> np.ndarray:
    ...
    """
    if filepath.startswith("s3://"):
        return np.array(easy_io.load(filepath)[0])
    else:
...
def resize_image(image: np.ndarray, short_size: int = None) -> np.ndarray:
    ...
    if height <= width:
        height_new, width_new = short_size, int(
            width * short_size / height + 0.5
        )
...
def write_image(filepath: str, image: np.ndarray):
    ...
    if filepath.startswith("s3://"):
        ...
        return easy_io.dump(image, filepath)
...
def write_video(filepath: str, video: np.ndarray, fps: int = 24) -> None:
    ...
    if filepath.startswith("s3://"):
        return easy_io.dump(video, filepath, fps=fps, quality=10)
...
def pad_video_batch(
    ...
    frames_to_pad = (
        (align - (num_frames - 1) % align)
        if (num_frames - 1) % align != 0
        else 0
    )
