...
class ImageTokenizer(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        init_s3_backend(credentials=credentials)
        self._device = device
        ...
        self._enc_model = (
            load_encoder_model(checkpoint_enc, tokenizer_config, device).to(
                self._dtype
            )
        ...
        self._dec_model = (
            load_decoder_model(checkpoint_dec, tokenizer_config, device).to(
                self._dtype
            )
            if checkpoint_enc is not None
            else None
