        ...
        for i_level in range(self.num_resolutions):
            ...
            for _ in range(self.num_res_blocks):
                ...
                if curr_res in attn_resolutions:
                    attn.append(
                        CausalAttnBlock(block_in, num_groups=num_groups)
                    )
            down = nn.Module()
            down.block = block
            down.attn = attn
...
class DecoderFactorized(nn.Module):
    def __init__(
        ...
        **ignore_kwargs,
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                ...
                if legacy_mode:
                    ...
                    temporal_up = (
                        0 < i_level_reverse < self.num_temporal_ups + 1
                    )
        ...
        self.conv_out = nn.Sequential(
            CausalConv3d(
                block_in, out_ch, kernel_size=(1, 3, 3), stride=1, padding=1
            ),
            CausalConv3d(
                out_ch, out_ch, kernel_size=(3, 1, 1), stride=1, padding=0
            ),
        )
